...
import ASimulatorSystem.PinHasher;
...
public class AtmService {
    ...
    public Account authenticate(String cardNumber, String pin) {
        Account account = accounts.findByCardNumberForUpdate(cardNumber)
                .orElseThrow(() -> new IllegalArgumentException("Invalid card number or PIN."));
        if (account.getStatus() != Account.Status.ACTIVE) {
            throw new IllegalStateException("Account is " + account.getStatus().name().toLowerCase() + ".");
        }
        if (!PinHasher.matches(pin, account.getPinHash())) {
            ...
            throw new IllegalArgumentException(attempts >= MAX_FAILED_ATTEMPTS
                    ? "Too many failed attempts. Account blocked."
                    : "Invalid card number or PIN. Attempts left: " + (MAX_FAILED_ATTEMPTS - attempts));
        }
    ...
    public Account createAccount(String cardNumber, String pin, BigDecimal initialDeposit) {
        if (accounts.findByCardNumber(cardNumber).isPresent()) {
        ...
        if (initialDeposit.compareTo(BigDecimal.ZERO) > 0) {
            record(account, BankTransaction.Type.DEPOSIT, initialDeposit);
        }
        return account;
    }
    ...
    public BigDecimal withdraw(long accountId, BigDecimal amount) {
        ...
        if (amount.compareTo(MAX_WITHDRAWAL) > 0) {
            throw new IllegalArgumentException("Maximum withdrawal is Rs. 10,000.");
        }
        Account account = activeAccountForUpdate(accountId);
    ...
    public void changePin(long accountId, String currentPin, String newPin) {
        Account account = activeAccountForUpdate(accountId);
        if (!PinHasher.matches(currentPin, account.getPinHash())) {
